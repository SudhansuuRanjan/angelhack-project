...
const Blog = () => {
    ...
    const getBlog = (slug) => {
        console.log(slug, news.find((blog) => blog.id === slug))
        return news.find((blog) => blog.id === slug);
    }
        ...
        <div className='news-container w-full'>
            ...
            {loading ?
                ...
                : <div className='w-full'>
                    <div className='bg-[#0b0b0b] lg:py-16 md:py-20 py-20 relative'>
                        <div className='flex items-center justify-center text-center gap-3 flex-col mb-16'>
                            <h1 data-aos="fade-up" className='news-topic font-bold text-purple-500'>{getBlog(topicSlug).title}</h1>
                        ...
                        <div className='flex items-center gap-1 text-gray-400 mt-3'>
                            <p>{getBlog(topicSlug).tag}</p>
                            <BsDot />
                            <p>{getBlog(topicSlug).minutes} min read</p>
                        </div>
                        <div className='flex items-center gap-3 mt-4'>
                            <img loading='lazy' className='w-9 h-9 rounded-full' src={getBlog(topicSlug).authorImage} alt={getBlog(topicSlug).author} />
                            <p className='font-semibold text-gray-300'>{getBlog(topicSlug).author}</p>
                        </div>
                    </div>
                        </div>
                    ...
                    <div data-aos="fade-up" className="img-container">
                        <img className='blog-header-img' src={getBlog(topicSlug).image} alt="header-img" />
                    </div>
                    <div className="paragraph-body w-full">
                        {
                            getBlog(topicSlug).body.map((para, index) => (
                                <p key={index}>{para}</p>
                            ))
                        }
                    </div>
                    {/* <div className="credits">
            <p>This article is contributed by <span>{getBlog(topicSlug).author}</span>.</p>
          </div> */}
                </div>
            }
            <BackButton to="/blogs" />
            {!loading && <div className='share-cont'>
                <Share topic={getBlog(topicSlug).title} para={getBlog(topicSlug).description} />
            </div>}
        </div>
    )
}
